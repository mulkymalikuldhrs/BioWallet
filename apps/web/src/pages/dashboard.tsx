...
export default function Dashboard() {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Handle PRF entropy for secure key derivation
      let entropySource = authResult.credentialId;
      const prfResults = (authResult as any).clientExtensionResults?.prf;

      if (prfResults?.results?.first) {
        entropySource = ethers.hexlify(new Uint8Array(prfResults.results.first));
      } else {
        const localSecret = localStorage.getItem('biowallet_local_secret');
        if (localSecret) {
          entropySource = ethers.keccak256(ethers.toUtf8Bytes(authResult.credentialId + localSecret));
        }
      }
