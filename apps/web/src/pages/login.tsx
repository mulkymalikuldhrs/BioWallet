  ...
  const handleLogin = async () => {
    try {
      ...
      if (result.success && result.credentialId) {
        ...
        let entropySource = result.credentialId;
        const prfResults = (result as any).clientExtensionResults?.prf;

        if (prfResults?.results?.first) {
          entropySource = ethers.hexlify(new Uint8Array(prfResults.results.first));
        } else {
          // Fallback to same logic as registration
          const localSecret = localStorage.getItem('biowallet_local_secret');
          if (localSecret) {
            entropySource = ethers.keccak256(ethers.toUtf8Bytes(result.credentialId + localSecret));
          }
        }
